"""
Pose data structures - Pose object and KEYPOINTS enum
Directly adapted from the original project, made standalone.
"""
...
class KEYPOINTS:
    """18 COCO-format body keypoint indices."""
    NOSE   = 0
    NECK   = 1
    R_SHO  = 2
    R_ELB  = 3
    ...
    @classmethod
    def all(cls):
        return list(range(cls.NUM_KPTS))


class Pose:
    """
    Represents a single person's detected pose.

    Attributes:
        keypoints: (18, 2) array of (x, y) coordinates. -1 means not detected.
        confidence: overall detection confidence score
        color: BGR color used for visualization
    """
    num_kpts = 18
    kpts = KEYPOINTS
    kpt_names = [
    ...
    kpt_groups = {
        'left': [
            ...
            KEYPOINTS.L_SHO, KEYPOINTS.L_ELB, KEYPOINTS.L_WRI,
            KEYPOINTS.L_HIP, KEYPOINTS.L_KNEE, KEYPOINTS.L_ANK,
            KEYPOINTS.L_EYE, KEYPOINTS.L_EAR
        ],
        'right': [
            KEYPOINTS.NOSE, KEYPOINTS.NECK,
            KEYPOINTS.R_SHO, KEYPOINTS.R_ELB, KEYPOINTS.R_WRI,
            KEYPOINTS.R_HIP, KEYPOINTS.R_KNEE, KEYPOINTS.R_ANK,
            KEYPOINTS.R_EYE, KEYPOINTS.R_EAR
        ]
    }
    sigmas = np.array(
    ...
    def draw(self, img, kpt_id_labels=False, kpt_coords=False):
        """Draw the skeleton overlay onto an image (in-place)."""
        ...
        if kpt_id_labels:
            for kpt_id in range(self.num_kpts):
                ...
                cv2.putText(
                    img, f"{kpt_id} {self.kpt_names[kpt_id]}",
                    (int(kpt_pos[0]), int(kpt_pos[1]) - 20),
        ...
        if kpt_coords:
            for kpt_id in range(self.num_kpts):
                ...
                cv2.putText(
                    img,
                    f"{self.kpt_names[kpt_id]} {int(kpt_pos[0]), int(kpt_pos[1])}",
                    (int(kpt_pos[0]), int(kpt_pos[1]) - 20),
    ...
    def to_dict(self):
        """Serialize keypoints to a dict keyed by keypoint name."""
        return {name: val.tolist() for name, val in zip(self.kpt_names, self.keypoints)}

    def get_kpt_group(self, group=None):
        """
        Return a new Pose containing only the keypoints in the given group.
        group must be 'left' or 'right'.
        """
        if group is not None:
            kps = copy.copy(self.keypoints)
            for i, kpt in enumerate(kps):
                if i not in self.kpt_groups[group]:
                    kps[i] = [-1, -1]
            return Pose(kps, self.confidence, self.color)
        return self


def get_similarity(a: Pose, b: Pose, threshold: float = 0.5) -> int:
    """Return the number of keypoints with OKS similarity above threshold."""
    num_similar_kpt = 0
...
def propagate_ids(previous_poses, current_poses, threshold: int = 3):
    """
    Propagate pose IDs across video frames to track individuals consistently.

    Args:
        previous_poses: Poses from the previous frame (already have IDs).
        current_poses: Poses from the current frame (will receive IDs).
        threshold: Minimum number of matching keypoints to link two poses.
    """
